...
Проект создан для того, чтобы при посещении банка идентифицировать клиента по фото и создать его финансовый профиль. На основании полученной информации о клиенте необходимо предложить максимально подходящие ему продукты. 
...
Ряд параметров финансового профиля клиента добавляют в общую сумму скоринга фиксированное кол-во баллов: от 0 до 15. 
При этом, чем **параметр важнее** - тем больше баллов он может добавить (это кол-во баллов также называется весом параметра). 
Максимальная сумма такого скоринга по каждому клиенту составляет **121**, минимальная - **0** баллов.


<a name="планы_на_будущее"></a> 
## Планы на будущее

- Добавление внешних API: yandex, др.
- Возможное добавление обработки фотографий с названиями на русском языке (пока она работает только с названиями на английском языке)
- Расширить модель скоринга, чтобы была возможность предлагать банковские продукты для юрлиц (например, если клиент - гендиректор) и/или портфеля из нескольких продуктов.
- Усовершенствовать модель скоринга для поллучения возможности учета связанных заемщиков.
- Добавить 3
